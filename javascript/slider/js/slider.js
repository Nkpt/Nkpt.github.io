function Slider(selector, options) {
    ...
    function dotLinks() {
        ...
        var listLength = imagesCount;
        console.log(listLength);
        var  tmpl = paginationNode.querySelector('.slider__pagination-item_tmpl');
        var fragment = document.createDocumentFragment();
        var cln;

        tmpl.style.display='';
        tmpl.classList.remove('slider__pagination-item_tmpl');
        for(var i=0; i<listLength;i++){
            cln = tmpl.cloneNode(true);
            cln.firstChild.dataset.slider__item = i;
            if(i===currentSlideIndex){cln.querySelector('a').classList.add('active');}
            //cln.dataset.slider__item = i;
            fragment.appendChild(cln);
        }
